...
| `PreloadSupplier` | No serialization | `export()` / `static from()` | Enables a server-loads-then-client-hydrates hand-off with no duplicate fetch — see `examples/react-ssr` | [`supplier/preload.ts`](packages/core/src/supplier/preload.ts) |
